...
fn copy<P: AsRef<Path>>(src_filename: &P, dst_filename: &P) -> Result<()> {
    ...
    let mut mp4_writer = mp4::Mp4Writer::write_start(
        writer,
        &Mp4Config {
            major_brand: *mp4_reader.major_brand().unwrap(),
            minor_version: mp4_reader.minor_version().unwrap(),
            compatible_brands: mp4_reader.compatible_brands().unwrap().to_vec(),
            timescale: mp4_reader.timescale().unwrap(),
        },
    )?;
